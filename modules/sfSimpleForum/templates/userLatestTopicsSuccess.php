...
<?php $title = __('Topics by %user%', array('%user%' => get_partial('sfSimpleForum/author_name', array('author' => $user->getFullName(), 'sf_cache_key' => $username))), 'sfSimpleForum') ?>
...
<div class="sfSimpleForum">
  ...
  <?php include_partial('sfSimpleForum/figures', array(
    ...
    'post_rule'           => 'sfSimpleForum/userLatestPosts?user_id='.$user->getUserId(),
    'feed_rule'           => 'sfSimpleForum/userLatestTopicsFeed?user_id='.$user->getUserId(),
    'feed_title'          => $feed_title
  )) ?>
    
  <?php include_partial('sfSimpleForum/topic_list', array('topics' => $topics_pager->getResults(), 'include_forum' => true)) ?>
  
  <?php echo pager_navigation($topics_pager, 'sfSimpleForum/userLatestTopics?user_id='.$user->getUserId()) ?>
