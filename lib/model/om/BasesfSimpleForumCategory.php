...
/**
 * Base class that represents a row from the 'sf_simple_forum_category' table.
 *
 * 
 *
...
abstract class BasesfSimpleForumCategory extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfSimpleForumCategoryPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        sfSimpleForumCategoryPeer
	 */
	...
	/**
	 * Initializes internal state of BasesfSimpleForumCategory object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     sfSimpleForumCategory The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->stripped_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 6; // 6 = sfSimpleForumCategoryPeer::NUM_COLUMNS - sfSimpleForumCategoryPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = sfSimpleForumCategoryPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasesfSimpleForumCategory:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
			...
			sfSimpleForumCategoryPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of sfSimpleForumCategory (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     sfSimpleForumCategory Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     sfSimpleForumCategoryPeer
	 */
	 ...
	 * Otherwise if this sfSimpleForumCategory has previously been saved, it will retrieve
	 * related sfSimpleForumForums from storage. If this sfSimpleForumCategory is new, it will return
		...
		if ($this->collsfSimpleForumForums === null) {
			if ($this->isNew()) {
			   $this->collsfSimpleForumForums = array();
				...
				$criteria->add(sfSimpleForumForumPeer::CATEGORY_ID, $this->id);
	 ...
	 * Otherwise if this sfSimpleForumCategory is new, it will return
	 * an empty collection; or if this sfSimpleForumCategory has previously
	 * been saved, it will retrieve related sfSimpleForumForums from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in sfSimpleForumCategory.
	 */
	public function getsfSimpleForumForumsJoinsfSimpleForumPost($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfSimpleForumCategoryPeer::DATABASE_NAME);
		}
		...
		if ($this->collsfSimpleForumForums === null) {
			if ($this->isNew()) {
				...
				$this->collsfSimpleForumForums = sfSimpleForumForumPeer::doSelectJoinsfSimpleForumPost($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
